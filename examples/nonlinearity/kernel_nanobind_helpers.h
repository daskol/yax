...
template <typename T> nanobind::bytes PackDescriptor(const T &descriptor) {
  std::string s = PackDescriptorAsString(descriptor);
  return nanobind::bytes(s.data(), s.size());
}

template <typename T> nanobind::capsule EncapsulateFunction(T *fn) {
  return nanobind::capsule(std::bit_cast<void *>(fn),
                           "xla._CUSTOM_CALL_TARGET");
}

template <typename T> nanobind::capsule EncapsulateFfiHandler(T *fn) {
  return nanobind::capsule(std::bit_cast<void *>(fn));
}
